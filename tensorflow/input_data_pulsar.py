...
def read_data(Candidates_pos_path,Candidates_neg_path,validation_part, test_part,cand_file_format = 'pfd'):
    ...
    train_subbands = [all_subbands[i] for i in samples_index[:int(Candidates_num*(1-validation_part-test_part))]]
    train_subints = [all_subints[i] for i in samples_index[:int(Candidates_num*(1-validation_part-test_part))]]
    train_DMs = [all_DMs[i] for i in samples_index[:int(Candidates_num*(1-validation_part-test_part))]]
    train_periods = [all_periods[i] for i in samples_index[:int(Candidates_num*(1-validation_part - test_part))]]
    train_labels = [all_labels[i] for i in samples_index[:int(Candidates_num*(1-validation_part - test_part))]]

    val_subbands = [all_subbands[i] for i in samples_index[int(Candidates_num*(1-validation_part - test_part)):int(Candidates_num*(1-validation_part))]]
    val_subints = [all_subints[i] for i in samples_index[int(Candidates_num*(1-validation_part-test_part)):int(Candidates_num*(1-validation_part))]]
    val_DMs = [all_DMs[i] for i in samples_index[int(Candidates_num*(1-validation_part-test_part)):int(Candidates_num*(1-validation_part))]]
    val_periods = [all_periods[i] for i in samples_index[int(Candidates_num*(1-validation_part-test_part)):int(Candidates_num*(1-validation_part))]]
    val_labels = [all_labels[i] for i in samples_index[int(Candidates_num*(1-validation_part-test_part)):int(Candidates_num*(1-validation_part))]]

    test_subbands = [all_subbands[i] for i in samples_index[int(Candidates_num*(1-validation_part )):]]
    test_subints = [all_subints[i] for i in samples_index[int(Candidates_num*(1-validation_part)):]]
    test_DMs = [all_DMs[i] for i in samples_index[int(Candidates_num*(1-validation_part)):int(Candidates_num*(1-validation_part))]]
    test_periods = [all_periods[i] for i in samples_index[int(Candidates_num*(1-validation_part)):]]
    test_labels = [all_labels[i] for i in samples_index[int(Candidates_num*(1-validation_part)):]]

    datasets.train = Dataset(train_subbands,train_subints,train_DMs,train_periods, train_labels,data_size,is_fake_data=False)
    datasets.validation = Dataset(val_subbands,val_subints,val_DMs,val_periods,val_labels,data_size,is_fake_data=False)
...
if __name__ == '__main__':
    ...
    validation_part = 0.1
    test_part = 0.2
    all_data = read_data(p_path, n_path,validation_part = validation_part, test_part= test_part)
